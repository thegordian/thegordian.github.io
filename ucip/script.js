...
var geoJsonLayerNode = L.geoJson(node_data,
    {
        ...
        pointToLayer: function (feature, latlng) {
            ...
            return L.circleMarker(latlng, {
                radius: (return_node_size_max(feature.properties.coverage,22)),//return_node_size
        ...
        onEachFeature: function (featureData, featureLayer) {
            ...
            featureLayer.on('click', function (e) {
                ...
                clickFeature_node(e, featureData)
...
geoJsonLayerNode_eval = L.geoJson(node_data,
    {
        ...
        pointToLayer: function (feature, latlng) {
            ...
            //feature._leaflet_id = feature.properties.node_id;
            return L.circleMarker(latlng, {
                radius: (return_node_size(feature.properties.coverage)),//return_node_size
                fillColor: node_color,
                color: "#000",
                weight: 1,
                opacity: 1,
...
function load_node_default() {
    ...
    geoJsonLayerNode.eachLayer(function (layer, latlng) {
        layer.setStyle({
            radius: (return_node_size_max(layer.feature.properties.coverage, 22)),
            fillColor: node_color,
            color: "#000",
            weight: 1,
            opacity: 1,
...
function highlightFeature_node(e) {
    if (hover_node) {
        ...
        $("#hover_info").html(
            '<b>' + e.target.feature.properties.tr_start_count + "</b>&nbsp; trajectories start from here, " +
            '&nbsp;<b>' + e.target.feature.properties.tr_end_count + "</b>&nbsp; trajectories end here," +
             "&nbsp; Total coverage"+'<b>&nbsp;' + e.target.feature.properties.coverage +"</b>"
        );
...
function resetHighlight_node(e) {
    ...
    if (selected_node_ids.includes((e.target._leaflet_id) -1)) {
        var layer = e.target;

        layer.setStyle(
            {
                radius: (return_node_size_max(e.target.feature.properties.coverage, 22)),
...
function clickFeature_node(e, featureData) {
    console.log('Clicked segment ID: ' + featureData.properties.node_id);

    lyr = e.target;

    if (selected_node_ids.indexOf(featureData.properties.node_id) === -1) {
        if (!multiple_selection) {
            if (selected_node_ids.length > 0) {
                for (var x = 0; x < selected_node_ids.length; x++) {
                    feature = geoJsonLayerNode.getLayer(selected_node_ids[x]);
                    feature.setStyle(
                        {
                            radius: (return_node_size_max(featureData.properties.coverage, 22)),
                            fillColor: selected_color,
                            color: "#000",
                            weight: 1,
                            opacity: 1,
        ...
        selected_node_ids.push(featureData.properties.node_id);
        ...
        index_of = selected_node_ids.indexOf(featureData.properties.node_id);
        selected_node_ids.splice(index_of, 1);
        lyr.setStyle(
            {
                //'weight': 3,
                'color': 'white',
    ...
    if (selected_node_ids.length > 0) {
        ...
        for (i = 0; i < selected_node_ids.length; i++) {
            ...
            search_val_array = node_tr_mapping[parseInt(selected_node_ids[i])];
            search_val_array_agg = union_arrays(search_val_array_agg, search_val_array);
        }

    }

    if (selected_segment_ids.length == 0 & selected_node_ids.length == 0)
    {
    ...
    console.log('Total unique trajectories:' + search_val_array_agg.length);
    $('#hover_info_right').text('Total unique trajectories:' + search_val_array_agg.length)
...
carto_black=L.tileLayer('https://cartodb-basemaps-{s}.global.ssl.fastly.net/dark_all/{z}/{x}/{y}.png', {'attribution': 'Map tiles by Carto, under CC BY 3.0. Data by OpenStreetMap, under ODbL.'})

var baseMaps = {
    "Grayscale": carto_black
};


var overlayMaps = {
    "segments": geoJsonLayerSeg
};
...
$(document).ready(function () {
    ...
    $('#filter_modal_toggle_btn').on('click', function () {
        ...
        if (selected_segment_ids_opt.length > 0) {
            ...
            geoJsonLayerNode_opt.eachLayer(function (layer, latlng) {
                layer.setStyle({
                    radius: (return_node_size_max(layer.feature.properties.coverage, max_node)),
                    fillColor: node_color,
                    color: "#000",
                    weight: 1,
                    opacity: 1,
